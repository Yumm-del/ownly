...
点击底部「**扫描空间**」，可以拍摄房间、柜子或桌面。Vision Adapter 会一次生成多个候选物品、与现有物品仓去重，并将低置信度结果留给用户确认；预览阶段不会污染长期记忆，确认后才批量写入同一个 ownership graph。公开 Demo 使用确定性 Adapter，接口可替换为本地视觉模型或 Agent OS 摄像头能力。
...
仓库包含 `render.yaml`，可以在 Render 中创建 Blueprint 并直接部署。服务会读取平台提供的 `PORT`、监听公网网卡，并通过 `/api/health` 接受健康检查。

免费实例的文件系统是临时的，因此重启或重新部署后 SQLite 会重新生成演示数据。这适合公开比赛 Demo；如需保存真实用户数据，请为服务挂载持久化磁盘并将 `OWNLY_DATA_DIR` 设为磁盘目录，或迁移到托管数据库。
